            ...
            setBody(
                response.data.map((arrayValue, index) => (
                    <TableRow key={index}>
                        ...
                        <TableCell>{arrayValue.attack_type}</TableCell>
                        <TableCell>{arrayValue.attack_vector}</TableCell>
                        <TableCell>{arrayValue.attack_value}</TableCell>
                        <TableCell>{arrayValue.threshold}</TableCell>
    ...
    const flowAggDiffPost = () => {
        axios.post('http://127.0.0.1:8000/sdn_communication/attack_notification/',{
            data: {
                ...
                'endDateMonth' : endDate.getMonth() + 1,
                'endDateDay' : endDate.getDate(),
                'filter' : 'Flow Aggregate Difference',
            },
        })
        .then((response) => {
            // console.log(response)
